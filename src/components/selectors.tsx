  ...
  return (
    <div className="sticky z-10 w-full top-2">
      <Listbox value={props.location} onChange={props.setLocation}>
        <Listbox.Button className="relative flex items-center justify-between w-full p-2 text-lg font-bold text-left transition duration-200 ease-in-out border-black rounded-lg shadow-lg cursor-pointer sm:text-xl sm:px-4 bg-amber-900 text-amber-100 border-1 border-opacity-5 focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-2 focus-visible:outline-amber-500">
          ...
          {props.data.locations
            ...
            .map((locationItem, locationIndex) => (
              <Listbox.Option
                key={locationIndex}
                className={({ active }) =>
                  `relative block p-2 text-base text-gray-900 truncate cursor-default select-none sm:text-lg ${
